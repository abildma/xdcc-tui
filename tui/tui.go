...
import (
	...
	"github.com/pcarrier/xdcc"
	"github.com/pcarrier/xdcc/search"
)
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "space":
			...
			if m.mode == ModeResults {
				fmt.Printf("DEBUG: Space pressed in results mode, cursor: %d, page: %d\n", m.cursor, m.page)
				...
				fmt.Printf("DEBUG: Calculated index: %d (startIdx: %d + cursor: %d)\n", actualIdx, startIdx, m.cursor)
				fmt.Printf("DEBUG: Results length: %d, Current page items: %d\n", len(m.filteredResults), endIdx-startIdx)
				...
				if actualIdx >= 0 && actualIdx < len(m.filteredResults) {
					...
					// Toggle selection flag directly on the item
					item.selected = !item.selected
					fmt.Printf("DEBUG: Toggled selection for '%s' to: %v\n", item.name, item.selected)
					
					// Also update the selection map
					m.selectedFiles[actualIdx] = item.selected
...
func (m Model) View() string {
		...
		if len(m.filteredResults) == 0 {
			...
			for i := startIdx; i < endIdx; i++ {
				...
				s.WriteString(fmt.Sprintf("%s [%s] %s (%d KB)\n", 
					cursor, selected, item.name, item.size/1024))
			}
		}
	...
	case ModeFilter:
		...
		s.WriteString("\nEnter: Apply filter | esc: Cancel | esc: Quit\n")
